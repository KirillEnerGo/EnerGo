...
import com.mosenergo.energo.constants.FirebaseAuthConstants.ERROR_USER_NOT_FOUND
...
//					Log.d("MyLog", "Exception : ${exception.errorCode}")
					...
					if (task.exception is FirebaseAuthUserCollisionException){
						...
						if (exception.errorCode == FirebaseAuthConstants.ERROR_EMAIL_ALREADY_IN_USE){
							...
							Toast.makeText(act, "Пользователь с таким e-mail уже существует, можете востановить пароль", Toast.LENGTH_LONG).show()
						}
						...
						if (exception.errorCode == FirebaseAuthConstants.ERROR_INVALID_EMAIL){
							...
							Toast.makeText(act, "Вы ввели некорректный e-mail", Toast.LENGTH_LONG).show()
//							Log.d("MyLog", "Exception : ${exception.errorCode}")
						}
					}
			...
			act.mAuth.signInWithEmailAndPassword(email, password).addOnCompleteListener { task->
				if (task.isSuccessful){
					...
					if (task.exception is FirebaseAuthInvalidCredentialsException) {
						...
						val exception = task.exception as FirebaseAuthInvalidCredentialsException
						Log.d("MyLog", "Exception 2:  ${exception.errorCode}") // отслеживать конкретную ошибку error code
						if (exception.errorCode == FirebaseAuthConstants.ERROR_INVALID_EMAIL){
							Toast.makeText(act, FirebaseAuthConstants.ERROR_INVALID_EMAIL, Toast.LENGTH_SHORT).show()
							Toast.makeText(act, "Вы ввели неправильный e-mail", Toast.LENGTH_LONG).show()
							...
							Toast.makeText(act, "Вы ввели неправильный пароль, можете востановить пароль", Toast.LENGTH_LONG).show()
						}
						...
						if (exception.errorCode == FirebaseAuthConstants.ERROR_USER_NOT_FOUND){
							...
							Toast.makeText(act, "Пользователь не зарегестрирован, проверьте логин / пароль или пройдите регистрацию", Toast.LENGTH_LONG).show()
						}
					}
